...
def _annotate_ast_nodes(ast_node: ast.AST) -> AnnotatedAst:
    ...
    for node in ast.walk(aast_node):
        ...
        if (
            ...
            delta = (node.lineno - 1, node.col_offset)
        node.startpos = startpos + delta
        if node.end_lineno is not None:
            node.endpos = startpos + (node.end_lineno - 1, node.end_col_offset)
    return aast_node
...
def _split_code_lines(
    ...
    for node, next_node in zip(ast_nodes, ast_nodes[1:] + [end_sentinel]):
        ...
        if next_startpos.lineno <= node.endpos.lineno:
            ...
            tail = text[end_lineno][node.endpos.colno - 1:]  # type: ignore[index]
            while (tail.endswith("\\") and "#" not in tail
                   and end_lineno < next_startpos.lineno):
                end_lineno += 1
                tail = text[end_lineno]  # type: ignore[assignment]
...
class PythonBlock:
    ...
    def string_literals(self) -> Iterator[Any]:
        ...
        nodes.sort(key=lambda node: node.startpos)
        yield from nodes
    ...
    def _get_docstring_nodes(self) -> Iterator[Any]:
        ...
        found.sort(key=lambda node: node.startpos)
        yield from found
